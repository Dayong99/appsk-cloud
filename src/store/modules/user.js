...
const user = {
  state: {
    ...
    avatar: '',
    permissionList: [],
    info: JSON.parse(localStorage.getItem('infouser'))||{},
    newValue:localStorage.getItem('entlong')||'',
    vals:'',
    name:'',
    roldID:localStorage.getItem('roldID')||''
  },
  ...
  mutations: {
    ...
    SET_INFO: (state, info) => {
      // console.log(JSON.parse(JSON.stringify(state.info)))
      state.info = info
      window.localStorage.setItem('infouser', JSON.stringify(state.info))
      // console.log(localStorage.getItem('infouser'))
    },
    SET_TENANT: (state, id) => {
      state.tenantid = id
    },
    SET_VLUE: (state, valueID) => {
      state.newValue = valueID
      window.localStorage.setItem('entlong', state.newValue)
    },
    ROLE_ID: (state, roldID) => {
      state.roldID = roldID
      window.localStorage.setItem('roldID', state.roldID)
    }
  },
  ...
  actions: {
      ...
      return new Promise((resolve, reject) => {
        login(userInfo).then(response => {
          ...
          if(response.code =='200'){
            ...
            commit('SET_INFO', userInfo)
            commit('SET_NAME', { username: userInfo.username,realname: userInfo.realname, welcome: welcome() })
            commit('SET_VLUE', entapp.relTenantIds)
            commit('ROLE_ID', result.roleId)
            resolve(response)
          }else{
            reject(response)
          }
    ...
    Logout({ commit, state }) {
      return new Promise((resolve) => {
        ...
        window.localStorage.removeItem('entlong')
        window.localStorage.removeItem('roldID')
      ...
      return new Promise((resolve, reject) => {
        thirdLogin(param.token,param.thirdType).then(response => {
          if(response.code =='200'){
            ...
            commit('SET_NAME', { username: userInfo.username,realname: userInfo.realname, welcome: welcome() })
            commit('SET_AVATAR', userInfo.avatar)
            commit('SET_VALUE', userInfo.relTenantIds)
            resolve(response)
          }else{
            reject(response)
          }

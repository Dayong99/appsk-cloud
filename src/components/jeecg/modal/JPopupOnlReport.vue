      ...
      <a-form layout="inline" @keyup.enter.native="searchByquery">
        <a-row :gutter="24" v-if="showSearchFlag">
          <template v-for="(item,index) in queryInfo">
            <template v-if=" item.field==='rel_tenant_ids' ">
              <a-col :md="8" :sm="24" :key=" 'query'+index " v-show="false">
                <online-query-form-item :queryParam="queryParam" :item="item" :dictOptions="dictOptions"></online-query-form-item>
              </a-col>
            </template>
            <template v-else-if=" item.hidden==='1' ">
  ...
  export default {
    ...
    methods:{
      loadColumnsInfo(){
        ...
        httpGroupRequest(() => getAction(url), groupIdKey).then(res => {
          if(res.success){
            ...
            console.log(res.result.dictOptions)
      ...
      loadData(arg) {
        ...
        if(params.materials_name){
           params.materials_name='*'+params.materials_name+"*"
        }
        if(params.materials_number){
          params.materials_number='*'+params.materials_number+"*"
        }
        if(params.classify_type_name){
          params.classify_type_name='*'+params.classify_type_name+"*"
        }
        if(this.$store.state.user.info.relTenantIds != null && this.$store.state.user.info.relTenantIds !='null' && this.$store.state.user.info.relTenantIds !=''){
          params.rel_tenant_ids=this.$store.state.user.info.relTenantIds
        }
        ...
        httpGroupRequest(() => getAction(url, params), groupIdKey).then(res => {
          ...
          if (data) {
            ...
            //   this.dictOptions.material_unit.forEach(b=>{
            //     if(a.material_unit==b.value){
            //       a.material_unit=b.title
            //     }
            //   })
            // })

...
<script>
  ...
  export default {
    ...
    data(){
      return {
        uploadAction:window._CONFIG['domianURL']+"/sys/common/uploads",
        headers:{},
        fileList: [],
        newFileList: [],
        uploadGoOn:true,
          ...
          // }else{
            ...
            // for(var a=0;a<fileList.length;a++){
              ...
              // if(fileList[a].status === 'done' ) {
                // console.log(fileList[a].response.result.fileName)
                var fileJson = {
                  fileName:fileList[0].response.result.fileName,
                  filePath1:fileList[0].response.message,
                  fileSize:fileList[0].response.result.fileSize,
                  fileType:fileList[0].response.result.fileType,
                  filePath:fileList[0].response.result.filePath
                };
              ...
              // update-end-author:lvdandan date:20200603 for:【TESTA-514】【开源issue】多个文件同时上传时，控制台报错
            // }
            this.$emit('change', this.newFileList[0].filePath1);
            this.$emit('changepng', this.newFileList);
          }
        }
      },
      handleDelete(file){

<template>
  ...
  <a-select v-else-if="tagType=='select'" :getPopupContainer = "getPopupContainer" :placeholder="placeholder" :disabled="disabled" :value="getValueSting" @change="handleInput">
    ...
    <a-select-option v-for="(item, key) in dictOptions" :key="key" :value="item.value" @click="getParameter">
  ...
  export default {
    ...
    methods: {
      initDictData() {
        ...
        let dictCodes='';
        if(this.dictCode.indexOf('rel_tenant_ids')==-1){
            dictCodes=this.dictCode+',rel_tenant_ids='+this.$store.state.user.newValue
        }

...
export const JeecgListMixin = {
  ...
  methods:{
    loadData(arg) {
      ...
      if(this.url.list=='/beamyard/structStatus/list'){
        delete params.column;
        delete params.order;
      }
      if(this.url.list=='/sys/planMain/list' && this.description == "质量巡检"){
        let key = 'orderType'
        let val = 0
        params[key] = val
      }
      ...
      if(this.url.list=='/deviceManage/device/list'){
        params = {
      ...
      getAction(this.url.list, params).then((res) => {
        if (res.success) {
          ...
          if(this.url.list=='/materials/weighbridgeBill/list'){
    ...
    handleAdd: function (val) {
      ...
      if(val==5){
        ...
        for (var a = 0; a < this.selectionRows.length; a++) {
          ids += this.selectionRows[a].createTime+","+ this.selectionRows[a].id +","+ this.selectionRows[a].bridgeName +","+ this.selectionRows[a].beamLengthDifference+";";
        }
        this.$store.state.user.vals=val;
        this.$refs.modalForm1.add(ids);
        this.$refs.modalForm1.title = "打印二维码";
    ...
    handleAddcreased: function (val,record) {
      //console.log(record)
      this.$store.state.user.vals=val;
    ...
    handleAddLog: function (val,record) {
      //console.log(record)
      this.$store.state.user.vals=val;
      this.$refs.modalLogForm.add(record);
      this.$refs.modalLogForm.title = "新增";
      this.$refs.modalLogForm.disableSubmit = false;
    },
    ...
    handleDetailLog:function(record){
      this.$refs.modalLogForm.edit(record);
      this.$refs.modalLogForm.title="详情";
      this.$refs.modalLogForm.disableSubmit = true;
    },
    /* 导出 */
    handleExportXls2(){

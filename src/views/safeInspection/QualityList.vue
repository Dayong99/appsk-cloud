<template>
  <a-card :bordered="false">
          ...
          <template v-if="toggleSearchStatus">
            <!-- <a-col :xl="6" :lg="7" :md="8" :sm="24">
              <a-form-item label="隐患类别">
                <j-dict-select-tag placeholder="请选择隐患类别" v-model="queryParam.hazardTypeId" dictCode="aoz_inspection_hazard_type,hazard_type_name,id"/>
              </a-form-item>
            </a-col>
            <a-col :xl="6" :lg="7" :md="8" :sm="24">
              <a-form-item label="隐患事件">
                <j-dict-select-tag placeholder="请选择隐患事件" v-model="queryParam.hazardContentId" dictCode="aoz_inspection_hazard_library,hazard_content,id"/>
              </a-form-item>
            </a-col> -->
            <a-col :xl="6" :lg="7" :md="8" :sm="24">
              <a-form-item label="问题分类">
                <j-dict-select-tag placeholder="请选择问题分类" v-model="queryParam.inspectProblemClassify" dictCode="aoz_sys_inspection_quality_question,quality_question,id"/>
              </a-form-item>
            </a-col>
            <a-col :xl="6" :lg="7" :md="8" :sm="24">
    ...
    <div class="table-operator">
      ...
      <!-- <a-button type="primary" icon="download" @click="handleExportXls('质量安全')">导出</a-button>
    ...
    <quality-modal :pollingResult="pollingResult" ref="modalForm" @ok="modalFormOk"></quality-modal>
  </a-card>
</template>
...
import QualityModal from './modules/QualityModal'
...
export default {
    ...
    return {
      ...
      columns: [
        ...
        // },
        {
          title: '问题分类',
          align: 'center',
          dataIndex: 'inspectProblemClassify_dictText',
        },
        {
          title: '整改期限',
          align: 'center',
      ...
      url: {
        list: '/sys/qualitySafety/list?orderType=0',
        delete: '/sys/qualitySafety/delete',
        deleteBatch: '/sys/qualitySafety/deleteBatch',
        exportXlsUrl: '/sys/qualitySafety/exportXls',
  ...
  methods: {
    ...
    getSuperFieldList() {
      ...
      // fieldList.push({type:'string',value:'hazardTypeId',text:'隐患类别',dictCode:'aoz_inspection_hazard_type,hazard_type_name,id'})
      // fieldList.push({type:'string',value:'hazardContentId',text:'隐患事件',dictCode:'aoz_inspection_hazard_library,hazard_content,id'})
      fieldList.push({
        type: 'int',
        value: 'inspectProblemClassify',
        text: '问题分类',
        dictCode: 'aoz_sys_inspection_quality_question,quality_question,id',
      })
    ...
    pollingQualityProblem() {
      let that = this;
      //巡检页跳转新增
      if (this.$route.query.polling != '' && typeof this.$route.query.polling != 'undefined' ) {
